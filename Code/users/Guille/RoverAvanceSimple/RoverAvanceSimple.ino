// Initialize
String vers = "Rover Avance Simple";
...
void loop()
{
Espera = millis()/1000 + SegundosEspera;

while(millis()/1000 < Espera){
  Dis=distancia(BajoTrig,BajoEcho);
  Serial.println(Dis);
  if ((Dis<12)) {
   Esquiva();
...
void Esquiva(){
  ...
  delay(1000);
  if(random(2) == 0) {
    GiroIzq(random(400, 1600));
  }
  else{
   GiroDer(random(400, 1600)); 
  }
