...
int tiempoEspera = 0;
unsigned long int tiempoParaAccion = 0;
...
void comportamiento(){
  Desviar();
  Esquiva();
  if(millis() > tiempoParaAccion){
    Para();
    acciones(random(100));
    tiempoParaAccion = millis() + tiempoEspera;
  }
}//comportamiento

void acciones(int comando){
  ...
  switch (comando){
    case 0 ... 49:
      ...
      Avanza(10);
      tiempoEspera = random(3, 11)*400;
      break;
    case 50 ... 54:
      //Retrocede
      Serial.println("Retrocede");
      Retrocede(10);
      tiempoEspera = random(4, 13)*400;
      break;

    case 55 ... 75:
      //Arco Der
      Serial.println("Arco Der");
      ArcoDer(10);
      tiempoEspera = random(3,16)*120;
      break;

    case 76 ... 96:
      //Arco Izq
      Serial.println("Arco Izq");
      ArcoIzq(10);
      tiempoEspera = random(3,16)*120;
      break;
...
void Desviar(){
    ...
    while (distancia(BajoTrig,BajoEcho) < 20){
      if(direccion == 0){
        ArcoDer(1);
      } else {
        ArcoIzq(1);
...
void Esquiva(){
  ...
  if(distancia(BajoTrig,BajoEcho) < 8)
  {
    Serial.print("Esquiva ");
    if(random(2) == 0) {
      Serial.println("der");
      GiroDer(600);
    }
